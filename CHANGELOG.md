...
- **`hostMatchesPattern(host, pattern)`** — Caddy host-pattern matcher: exact, single-label leading wildcard (`*.example.com` matches `foo.example.com` but not `example.com` or `a.b.example.com`), generic glob (`api-*.example.com`, `*-prod`, `*.api-*.example.com`). The single-label branch only fires when the tail after `*.` is a literal — patterns like `*.api-*.example.com` correctly take the generic-glob path.
- **`buildAcmeDnsPolicy({ subjects, dnsProvider, ca?, email?, providerConfig? })`** — emits the wrapper shape Caddy expects: `{ subjects, issuers: [{ module: "acme", challenges: { dns: { provider: { name } } }, … }] }`. The `provider.name` discriminator is what Caddy core actually expects (`caddytls/automation.go`: `caddy:"namespace=dns.providers inline_key=name"`); `module` is reserved for the issuer level, not provider sub-modules. Common shortcuts (`cloudflare`, `porkbun`, `route53`, `digitalocean`, `godaddy`) map to module names; unknown names pass through. Inputs are normalised (trim + lowercase) so `" Cloudflare "` resolves to `cloudflare`. `providerConfig` is an opaque passthrough — `name` is reserved (overriding the discriminator throws).
- **`ACME_DNS_PROVIDER_MODULE_MAP`** + **`resolveAcmeDnsProviderModule(name)`** — frozen lookup that backs `buildAcmeDnsPolicy`. Resolver normalises input (trim + lowercase) before lookup.
- **`filterAcmeManagedFromSkip(candidates, acmeManagedHosts)`** — drops `automatic_https.skip` entries that would shadow an external-ACME policy. Three-way check: (1) exact lower-case match, (2) candidate is a wildcard matching any ACME-managed host, (3) ACME-managed host is a wildcard covering the candidate. Returns a fresh array (no aliasing input).
- **`applyLocalCaInstallTrust(config, value)`** — sets `apps.pki.certificate_authorities.local.install_trust` on a resolved Caddy config. Idempotent. Throws when an intermediate node exists but is not a plain object — surfacing caller-side config corruption rather than silently overwriting it. Motivating case: Windows readiness hang where Caddy's local CA triggers UAC + a `certutil` invocation on first issuance, blocking the admin API for 5–30 s.

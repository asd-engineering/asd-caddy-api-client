...
import { detectContext as detectContextPure } from "./completion-context";
...
export class CaddyCompletionProvider implements vscode.CompletionItemProvider {
  ...
  private detectContext(
    ...
    // Check for builder context first (TypeScript/JavaScript files) -- this
    // genuinely needs vscode's own word-range tokenizer, so it stays here
    // rather than in the pure completion-context.ts module.
    if (this.isBuilderContext(document, position)) {
      return { type: "builder" };
    }

    return detectContextPure({
